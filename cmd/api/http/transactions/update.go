...
func (r *TransactionsRouter) UpdateRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful transaction update.").
	...
	body := &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithDescription("Payload to update an existing transaction.").
			WithContent(openapi3.Content{
				"application/json": openapi3.NewMediaType().
					WithSchema(schemas.UpdateTransactionSchema.Value).
					WithExample("example", schemas.UpdateTransactionSchema.Value),
			}),
	}
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			var payload models.UpdateTransactionPayload
			...
			if err := r.Services.Transactions().Update(params.Id, payload); err != nil {
