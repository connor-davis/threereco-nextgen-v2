...
func (r *TransactionMaterialsRouter) DeleteRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful transaction material deletion.").
	...
	return routing.Route{
		OpenAPIMetadata: routing.OpenAPIMetadata{
			Summary:     "Delete Transaction Material",
			Description: "Delete an existing transaction material from the system.",
		...
		Middlewares: []fiber.Handler{
			...
			r.Middleware.Authorized([]string{"transactions.materials.delete"}),
		},
		Handler: func(c *fiber.Ctx) error {
			...
			if err := r.Services.Transactions().Materials().Delete(params.Id); err != nil {
