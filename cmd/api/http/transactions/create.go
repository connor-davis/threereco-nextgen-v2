...
func (r *TransactionsRouter) CreateRoute() routing.Route {
	...
	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful transaction creation.").
	...
	body := &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithDescription("Payload to create a new transaction.").
			WithContent(openapi3.Content{
				"application/json": openapi3.NewMediaType().
					WithSchema(schemas.CreateTransactionSchema.Value).
					WithExample("example", schemas.CreateTransactionSchema.Value),
			}),
	}
	...
	return routing.Route{
		...
		Middlewares: []fiber.Handler{
			...
			r.Middleware.Authorized([]string{"transactions.create"}),
		},
		Handler: func(c *fiber.Ctx) error {
			...
			if err := r.Services.Transactions().Create(payload); err != nil {
