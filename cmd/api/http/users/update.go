...
func (r *UsersRouter) UpdateRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful user update.").
	...
	body := &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithDescription("Payload to update an existing user.").
			WithContent(openapi3.Content{
				"application/json": openapi3.NewMediaType().
					WithSchema(schemas.UpdateUserSchema.Value).
					WithExample("example", schemas.UpdateUserSchema.Value),
			}),
	}
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			if err := r.Services.Users().Update(params.Id, payload); err != nil {
