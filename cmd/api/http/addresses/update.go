...
func (r *AddressesRouter) UpdateRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful address update.").
	...
	body := &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithDescription("Payload to update an existing address.").
			WithContent(openapi3.Content{
				"application/json": openapi3.NewMediaType().
					WithSchema(schemas.UpdateAddressSchema.Value).
					WithExample("example", schemas.UpdateAddressSchema.Value),
			}),
	}
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			var payload models.UpdateAddressPayload
			...
			if err := r.Services.Addresses().Update(params.Id, payload); err != nil {
