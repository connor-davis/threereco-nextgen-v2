...
func (r *MaterialsRouter) UpdateRoute() routing.Route {
	responses := openapi3.NewResponses()

	responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			WithDescription("Successful material update.").
	...
	body := &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithDescription("Payload to update an existing material.").
			WithContent(openapi3.Content{
				"application/json": openapi3.NewMediaType().
					WithSchema(schemas.UpdateMaterialSchema.Value).
					WithExample("example", schemas.UpdateMaterialSchema.Value),
			}),
	}
	...
	return routing.Route{
		...
		Handler: func(c *fiber.Ctx) error {
			...
			var payload models.UpdateMaterialPayload
			...
			if err := r.Services.Materials().Update(params.Id, payload); err != nil {
